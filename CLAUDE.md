...
- `exec.ts` — wraps Node's `child_process.spawn` in a promise. Each of stdout and stderr accepts an `OutputMode`: `"inherit"` (default, passes output to the current process; stdout also logs the command via `logCommand`), `"capture"` (collects and returns the output as a string), or `"silent"` (suppresses the output). Modes can be set independently per stream.
